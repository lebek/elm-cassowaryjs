...
-- closestVertex : Nonempty Vertex -> Position -> (Int, Vertex)
-- closestVertex poly position =
--   let
--     indexed = NE.indexedMap (\i v -> (i, v)) poly
--     f (i, v) (mini, minv) =
--       if (distance v.position position) < (distance minv.position position) then (i, v) else (mini, minv)
--   in
--   List.foldr f (NE.head indexed) (NE.tail indexed)

closestVertex : Nonempty Vertex -> Position -> Vertex
closestVertex poly position =
  let
    f v minv =
      if (distance v.position position) < (distance minv.position position) then v else minv
  in
  ...
  case msg of
    DragStart xy ->
      let
        ...
        --closest = closestVertex allVertex xy
        --updatedSolver = addEditVar (snd closest).vars.x solver
            --|> addEditVar (snd closest).vars.y
        vertex = closestVertex (NE.append outerPoly innerPoly) xy
        updatedSolver = addEditVar vertex.vars.x solver
          |> addEditVar vertex.vars.y
          |> beginEdit
      in
        Model outer inner (Just <| Drag xy xy vertex) updatedSolver

    DragAt xy ->
      let
        d = Debug.log "DragAt" xy
        updateSolver {start,current,closest} = suggestValue closest.vars.x (toFloat (closest.position.x + current.x - start.x)) solver
          |> suggestValue closest.vars.y (toFloat (closest.position.y + current.y - start.y))
          |> solve
      in case drag of
